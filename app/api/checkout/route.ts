  ...
  try {
    ...
    const line_items: Stripe.Checkout.SessionCreateParams.LineItem[] = items.map(
      (it) => ({
        ...
        price_data: {
          currency: "usd", // ✅ Make sure USD is enabled in your Stripe dashboard
    ...
    const session = await stripe.checkout.sessions.create({
      mode: "payment",
      payment_method_types: ["card"], // ✅ requires "card" to be enabled
      line_items,
      metadata: {
        user_id,
        cart: JSON.stringify(
          items.map(({ cart_id, product_id, quantity, price }) => ({
            cart_id,
            product_id,
            quantity,
            price,
